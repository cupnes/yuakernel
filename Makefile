...
run: $(TARGET)
	cp $(TARGET) ../fs/
	qemu-system-x86_64 -enable-kvm -cpu host -machine pc,accel=kvm -m 4G -hda fat:../fs -drive if=pflash,format=raw,readonly,file=$$HOME/ovmf/OVMF_CODE.fd -drive if=pflash,format=raw,file=$$HOME/ovmf/OVMF_VARS.fd
