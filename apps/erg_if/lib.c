...
struct framebuffer *get_fb(void)
{
	return (struct framebuffer *)syscall(SYSCALL_GET_FB, 0, 0, 0);
}
...
void fill_img(struct file *img_file)
{
	memcpy(fb->base, img_file->data, img_file->size);
}
